...
const PM_CATEGORIES = ['Design Documents', 'API Documents', 'Sprint Reports', 'Meeting Minutes', 'Architecture', 'Release Notes'];
...
export const Documents: React.FC = () => {
  ...
  if (profile?.role === 'Project Manager') {
    const mockPmDocs = [
      { id: '1', name: 'Auth Module V2', type: 'Design Documents', size: '2.4 MB' },
      { id: '2', name: 'Sprint 42 Report', type: 'Sprint Reports', size: '1.1 MB' },
      { id: '3', name: 'User API Spec', type: 'API Documents', size: '840 KB' },
      { id: '4', name: 'System Architecture Diagram', type: 'Architecture', size: '4.2 MB' },
      { id: '5', name: 'v1.4 Release Notes', type: 'Release Notes', size: '230 KB' },
    ];

    const filteredPmDocs = mockPmDocs.filter(doc => 
      doc.name.toLowerCase().includes(searchQuery.toLowerCase()) || 
      doc.type.toLowerCase().includes(searchQuery.toLowerCase())
    );
      ...
      <div className="space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500">Repository for project-related files</p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <Button onClick={() => {
              setUploadData({ ...uploadData, type: 'Design Documents' });
        ...
        <div className="space-y-8">
          {PM_CATEGORIES.map(category => {
            const categoryDocs = filteredPmDocs.filter(doc => doc.type === category);
        ...
        {/* Upload Modal (Reusing existing state but for PM) */}
        <Modal
          isOpen={isUploadModalOpen}
          onClose={() => setIsUploadModalOpen(false)}
